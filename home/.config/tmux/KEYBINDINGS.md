...
| `Ctrl-a` → `?` / `g` | **キーバインド一覧をポップアップ**（独自キーが上・fzf で絞り込み・`claude-keys.sh`）。2キーとも同じ動作。`g` = guide、Shift 不要 |
| `Ctrl-a` → `d` | デタッチ（tmux 標準） |

> **キーを増やしたら `-N` を付ける。** `Ctrl-a → ?` の一覧は tmux の `list-keys -N` を情報源に
> しており、**ノート（`-N`）が付いたキーしか載らない**。tmux.conf では自作バインドに
> `-N "◆ 説明"` を付ける約束にしてある（`◆` が「独自キー」の目印）。
> 例: `bind -N "◆ 新機能" X run-shell "…"` と書けば、一覧に自動で載る。
...
> `Ctrl+h / j / k / l` のペイン移動は**廃止**した。Shift+Enter が送る改行(LF)は `Ctrl-J` と
> 同一バイトのため、「`Ctrl-J` → 下ペインへ移動」がプロンプトの改行を横取りしていたため。
...
| `Ctrl-a` → `&` | **このウィンドウを閉じる**（中のペインごと終了・`y` で確定） |
| `Ctrl-a` → `x` | このペインだけ閉じる（`y` で確定） |

作業場（左=会話ログ / 右上=claude / 右下=shell）を畳むのは `&`。ウィンドウが1つだけの
ときに閉じるとセッションごと終了して端末に戻る。プロセスを残して抜けたいだけなら
`Ctrl-a` → `d`（デタッチ）。
...
| 何が起きたか | 犯人 | 戻し方 |
|---|---|---|
| 1ペインが全画面になり他が消えた | `Ctrl-a` → `z`（ズーム） | もう一度 `Ctrl-a` → `z`。ステータスバーの `Z` が目印 |
| ペインが1枚どこかへ行った | `Ctrl-a` → `!`（break-pane・**確認なし**） | 別ウィンドウへ移動している。`Alt+n` / `Alt+p` で探す |
| ペインが極端に細くなった | `Ctrl-a` → `H/J/K/L`（リサイズ・連打可） | 逆方向のキーで戻す |

`Ctrl-a` → `x`（kill-pane）と `&`（kill-window）は y/n の確認が出るので「突然」は起きない。
迷ったら `Ctrl-a` → `?` でキー一覧を開く。
